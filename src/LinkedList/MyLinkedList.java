...
public class MyLinkedList<E> /*implements List*/{
	...
	private boolean addFirst(E value){//在前面添加
		addAfter(new Node(value),head);
		return true;
	}
	private boolean addLast(E value){//在后面添加
		addBefore(new Node(value),head);
		return true;
	}


	private boolean add(E value){//默认在后面添加
		addLast(value);
		return true;
	}
	private boolean add(int index,E value){//在指定节点添加，即在指定节点之前添加
